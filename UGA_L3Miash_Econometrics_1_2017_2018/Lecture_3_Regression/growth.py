...
if os.path.exists('/Users/michalurdanivia/Documents/Données/Données_cours/ Growth.dta') == False:
...
plt.scatter(X1, results.predict(), alpha = 0.5, label = 'fitted')
...
plt.xlabel('rgdp60')
plt.ylabel('growth')
plt.savefig(workdir+'lreg_GrowthGDP.pdf')
plt.show()

X = sm.add_constant(X2)
lreg2 = sm.OLS(endog = Y, exog = X, missing ='drop')
type(lreg2)
results = lreg2.fit()
type(results)
print(results.summary())
...
results.predict(exog=[1, meanX2])

plt.scatter(X2, results.predict(), alpha = 0.5, label = 'fitted')

plt.scatter(X2, Y, alpha=0.5, label = 'observed')
plt.legend()
plt.title('OLS fitted values')
plt.xlabel('yearschool')
plt.ylabel('growth')
plt.savefig(workdir+'lreg_GrowthSchool.pdf')
plt.show()
